...
async function loadCategories() {
  try {
    if (searchSource.value === "modrinth") {
      categories.value = await invoke<UnifiedCategory[]>("get_modrinth_shaderpack_categories");
    } else {
      categories.value = await invoke<UnifiedCategory[]>("get_curseforge_shaderpack_categories");
    }
  } catch (err) {
...
async function performSearch(isLoadMore = false) {
  ...
  try {
    ...
    if (searchSource.value === "modrinth") {
      results = await invoke<UnifiedModProject[]>("search_modrinth_shaderpacks", args);
    } else {
      results = await invoke<UnifiedModProject[]>("search_curseforge_shaderpacks", args);
    }
...
async function startActualDownload(mod: UnifiedModProject, fileId: string, targetDir: string | null) {
  ...
  try {
    ...
    trackEvent("Shaderpack Install Completed", { name: mod.title, projectId: mod.project_id, versionId: latestFile.id });
  } catch (err) {
    trackEvent("Error Occurred", { context: "shaderpack_install", error_type: getErrorType(err) });

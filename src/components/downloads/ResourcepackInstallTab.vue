...
async function loadCategories() {
  try {
    if (searchSource.value === "modrinth") {
      categories.value = await invoke<UnifiedCategory[]>("get_modrinth_resourcepack_categories");
    } else {
      categories.value = await invoke<UnifiedCategory[]>("get_curseforge_resourcepack_categories");
    }
  } catch (err) {
...
async function performSearch(isLoadMore = false) {
  ...
  try {
    ...
    if (searchSource.value === "modrinth") {
      results = await invoke<UnifiedModProject[]>("search_modrinth_resourcepacks", args);
    } else {
      results = await invoke<UnifiedModProject[]>("search_curseforge_resourcepacks", args);
    }
...
async function startActualDownload(mod: UnifiedModProject, fileId: string, targetDir: string | null) {
  ...
  try {
    ...
    trackEvent("Resourcepack Install Completed", { name: mod.title, projectId: mod.project_id, versionId: latestFile.id });
  } catch (err) {
    trackEvent("Error Occurred", { context: "resourcepack_install", error_type: getErrorType(err) });
...
function startInstallToInstance() {
  if (!pendingMod.value) return;
  startActualDownload(pendingMod.value, selectedFileId.value, null);
}

...
async function loadCategories() {
  try {
    categories.value = await invoke<UnifiedCategory[]>("get_curseforge_world_categories");
  } catch (err) {
...
async function fetchModFilesForSelectedVersion() {
  ...
  selectedFileId.value = "";
    ...
    try {
      const files: UnifiedModFile[] = await invoke("get_cf_mod_files", {
...
async function performSearch(isLoadMore = false) {
  ...
  try {
    ...
    results = await invoke<UnifiedModProject[]>("search_curseforge_worlds", args);

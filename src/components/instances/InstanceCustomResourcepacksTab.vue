  ...
  <div class="flex-1 h-full w-full flex flex-col min-h-0 bg-white/40 dark:bg-zinc-900/40">
    <div class="px-6 py-4 border-b border-neutral-200/50 dark:border-zinc-800/50 flex-shrink-0 flex items-center justify-between">
      <h3 class="text-lg font-semibold flex items-center gap-2">
        ...
        {{ $t('instances.customResourcepacks', 'Custom Resource Packs') }}
      </h3>
    </div>
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-6 flex flex-col gap-4 minimal-scrollbar">
      <div class="flex items-center gap-3">
        <DInput
          v-model="searchQuery"
          :placeholder="$t('instances.searchCustomResourcepacks', 'Search custom resource packs...')"
          class="flex-1"
        >
          <template #prefix>
      ...
      <div v-else-if="filteredAssets.length === 0" class="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
        ...
        <p class="text-lg font-medium">{{ $t('instances.noCustomResourcepacks', 'No global custom resourcepacks found.') }}</p>
        <p class="text-sm mt-2 max-w-md">{{ $t('instances.customResourcepacksHint', 'You can place your favorite resourcepacks in the global_assets folder to quickly apply them to any instance.') }}</p>
      </div>
...
import { Box, Search, Trash2, FolderArchive, Folder, ArrowRight, FolderOpen, RefreshCw, Edit } from '@lucide/vue';
...
const ASSET_TYPE = 'resourcepacks';

  ...
  <div class="flex-1 h-full w-full flex flex-col min-h-0 bg-white/40 dark:bg-zinc-900/40">
    <div class="px-6 py-4 border-b border-neutral-200/50 dark:border-zinc-800/50 flex-shrink-0 flex items-center justify-between">
      <h3 class="text-lg font-semibold flex items-center gap-2">
        ...
        {{ $t('downloadsCenter.tabs.shader', 'Shaders') }}
      </h3>
    </div>
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-6 flex flex-col gap-4 minimal-scrollbar">
      <div class="flex items-center gap-3">
        <DInput
          v-model="searchQuery"
          :placeholder="$t('instances.searchShaders', 'Search installed shaders...')"
          class="flex-1"
        >
          <template #prefix>
      ...
      <div v-else-if="filteredAssets.length === 0" class="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
        ...
        <p class="text-lg font-medium">{{ $t('instances.noShaders', 'No shaders installed for this instance') }}</p>
      </div>
...
import { Sparkles, Search, Download, Trash2, FolderArchive, Folder } from '@lucide/vue';
...
async function loadAssets() {
  ...
  loading.value = true;
  try {
    const result = await invoke<LocalAssetItem[]>('get_installed_shaders', { versionId: props.instanceId });
    assets.value = result;
  } catch (error) {
...
async function executeDelete() {
  ...
  try {
    await invoke('delete_local_shader', {
...
function handleDownloadMore() {
  router.push({ path: '/downloads', query: { tab: 'shader', instanceId: props.instanceId } });
}

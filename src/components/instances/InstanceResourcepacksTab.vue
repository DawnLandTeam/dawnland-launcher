  ...
  <div class="flex-1 h-full w-full flex flex-col min-h-0 bg-white/40 dark:bg-zinc-900/40">
    <div class="px-6 py-4 border-b border-neutral-200/50 dark:border-zinc-800/50 flex-shrink-0 flex items-center justify-between">
      <h3 class="text-lg font-semibold flex items-center gap-2">
        ...
        {{ $t('instances.resourcepacks', 'Resource Packs') }}
      </h3>
    </div>
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-6 flex flex-col gap-4 minimal-scrollbar">
      <div class="flex items-center gap-3">
        <DInput
          v-model="searchQuery"
          :placeholder="$t('instances.searchResourcepacks', 'Search installed resource packs...')"
          class="flex-1"
        >
          <template #prefix>
        ...
        <DButton variant="primary" @click="handleDownloadMore">
          ...
          {{ $t('instances.downloadMore', 'Download More') }}
        </DButton>
      </div>
      ...
      <div v-else-if="filteredAssets.length === 0" class="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
        ...
        <p class="text-lg font-medium">{{ $t('instances.noResourcepacks', 'No resource packs installed for this instance') }}</p>
      </div>
...
import { Box, Search, Download, Trash2, FolderArchive, Folder } from '@lucide/vue';
...
async function loadAssets() {
  ...
  loading.value = true;
  try {
    const result = await invoke<LocalAssetItem[]>('get_installed_resourcepacks', { versionId: props.instanceId });
    assets.value = result;
  } catch (error) {
    console.error('Failed to load resourcepacks:', error);
    toast.error(
...
async function executeDelete() {
  ...
  try {
    await invoke('delete_local_resourcepack', {
    ...
    console.error('Failed to delete resourcepack:', error);
    toast.error(
      t('common.error', 'Error'),
      getErrorMessage(error)
    );
...
function handleDownloadMore() {
  router.push({ path: '/downloads', query: { tab: 'resourcepack', instanceId: props.instanceId } });
}
